...
1.⁠ ⁠Check if the task goal has already been completed (i.e., not just filled out, but fully finalized by CLICKING SAVE/SUBMIT. DON'T SAY TASK IS COMPLETED UNTIL THE SAVE BUTTON IS CLICKED). If so, return: null
...
4.⁠ ⁠Return:
    - A playwright code that will perform the next predicted step to get closer to the end goal.
...
Your response must be valid Python code that uses Playwright's page object. For example:
⁠ python
page.get_by_role("button", name="Create").click()
 ⁠
or
⁠ python
page.get_by_label("Event title").fill("Team Meeting")
 ⁠

Return ONLY the code, no explanations or markdown (ONE LINE OF CODE ONLY AT A TIME). If no further action is needed, return:
null"""
...
Your response must be valid Python code that uses Playwright's page object. For example:
⁠ python
page.get_by_role("button", name="Delete").click() or some other code to delete the event
 ⁠
...
4.⁠ ⁠Return:
    - A playwright code that will perform the next predicted step using a different strategy
...
Your response must be valid Python code that uses Playwright's page object. For example:
⁠ python
# If previous attempt using get_by_role failed, try get_by_label
page.get_by_label("Event title").fill("Team Meeting")
 ⁠
or
⁠ python
# If previous attempt failed due to timing, add wait
page.wait_for_selector('[aria-label="Event title"]')
page.get_by_label("Event title").fill("Team Meeting")
 ⁠
